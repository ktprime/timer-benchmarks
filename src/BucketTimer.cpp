...
//cancellation
bool BucketTimer::Erase(const int timer_id)
{
...
bool BucketTimer::Cancel(const int timer_id)
{
    const auto itnode = hash_.find(timer_id);
    if (itnode == hash_.end())
        return false;
...
int BucketTimer::Schedule(uint32_t deadline_ms, TimerCallback cb)
{
    auto node    = allocNode();
    node->cb     = cb;
...
void BucketTimer::Update(int deadline_ms, int timer_id, TimerCallback cb)
{
    auto node = allocNode();
    node->cb = cb;
...
bool BucketTimer::Delay(int deadline_ms, int timer_id)
{
    const auto itnode = hash_.find(timer_id);
    if (itnode == hash_.end())
        return false;
...
int BucketTimer::tick(ehset<int>& timer_info)
{
    timer_info_.clear();
...
int BucketTimer::Update(int64_t nowms)
{
    if (nowms == 0)
...
int BucketTimer::NearTime(const int slots)
{
